...
/**
 * SPViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
public class SPViewDragHelper {
    private static final String TAG = "SPViewDragHelper";
    ...
    /**
     * A Callback is used as a communication channel with the SPViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the SPViewDragHelper with more information
    ...
    public static abstract class Callback {
         ...
         * one of these methods, the SPViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the SPViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>SPViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new SPViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb Callback to provide information and receive events
     * @return a new SPViewDragHelper instance
     */
    public static SPViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new SPViewDragHelper(forParent.getContext(), forParent, null, cb);
    }

    /**
     * Factory method to create a new SPViewDragHelper with the specified interpolator.
     *
     * @param forParent Parent view to monitor
     ...
     * @return a new SPViewDragHelper instance
     */
    public static SPViewDragHelper create(ViewGroup forParent, Interpolator interpolator, Callback cb) {
        return new SPViewDragHelper(forParent.getContext(), forParent, interpolator, cb);
    }
     ...
     * @return a new SPViewDragHelper instance
     */
    public static SPViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final SPViewDragHelper helper = create(forParent, cb);
     ...
     * @return a new SPViewDragHelper instance
     */
    public static SPViewDragHelper create(ViewGroup forParent, float sensitivity, Interpolator interpolator, Callback cb) {
        final SPViewDragHelper helper = create(forParent, interpolator, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }

    /**
     * Apps should use SPViewDragHelper.create() to get a new instance.
     ...
     * @param interpolator interpolator for scroller
     */
    private SPViewDragHelper(Context context, ViewGroup forParent, Interpolator interpolator, Callback cb) {
        if (forParent == null) {
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant " +
                    "of the SPViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the SPViewDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEventCompat.ACTION_POINTER_DOWN: {
                ...
                // A SPViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
